...
const Home = () => {
    ...
    useEffect(() => {
        ...
        // Target state for Contact page
        const contactState = {
            value: 200,           // same as in Contact.js
            deltaAngle: 0.5,
            opacity: 0.9
        };
        const contactColor = { r: 1, g: 1, b: 1 };  // white
        ...
        .to(patternProgressRef.current, {
            ...
            // 1. Morph the geometry parameters over 2s
        .to(patternProgressRef.current, {
            value: contactState.value,
            deltaAngle: contactState.deltaAngle,
            opacity: contactState.opacity,
            duration: 5,
            ease: "power2.inOut",
            onUpdate: () => {
                patternRef.current.regenerate({
                ...
                // pass opacity through newParam
                });
            }
            })
            // 2. Simultaneously fade the color to white
            .to({}, {
                duration: 5,
                ease: "power2.inOut",
                onUpdate: function () {
                  ...
                  for (let i = 0; i < colors.length; i += 3) {
                    colors[i] += (1 - colors[i]) * progress;     // R
                    colors[i + 1] += (1 - colors[i + 1]) * progress; // G
                    colors[i + 2] += (1 - colors[i + 2]) * progress; // B
                  }
